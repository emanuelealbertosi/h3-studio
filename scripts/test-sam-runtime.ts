...
const windows = parseWindowsSamWorkers(JSON.stringify({
  ...
  Name: "python.exe",
  CommandLine: "python.exe C:\\Temp\\persistent_worker.py tcp://127.0.0.1 comfyui-sam3",
}));
...
const control = new SamRuntimeControl("win32", async (file, args) => {
  ...
  if (file === "powershell.exe") {
    statusCalls += 1;
    return {
      stdout: statusCalls === 1 ? JSON.stringify({
        ProcessId: 22812,
        ParentProcessId: 23008,
        Name: "python.exe",
        CommandLine: "python persistent_worker.py comfyui-sam3",
      }) : "",
      stderr: "",
    };
  }
