...
class H3AIOAutopromptRequest:
    ...
    def build(self, generation_mode, natural_prompt, reference_roles,
        ...
        if role == "music_video_lipsync":
            audio_rule = (
                "Audio 1 is the authoritative complete speech, vocal or music "
                "track for synchronized visible performance. In every shot "
                "description use the literal marker <Soundtrack> for the "
                "currently audible time-aligned slice. When speech or vocals are "
                "audible, describe precise visible lip movement, breathing and "
                "performance synchronized to <Soundtrack>; do not invent, quote "
                "or transcribe unheard words or lyrics. Set active_ref_audios to an "
                "empty array because the dedicated sampler injects the correct "
                "slice itself. Set every music field to N/A: the original audio "
                "is preserved unchanged as the final soundtrack, not regenerated "
                "by H3.")
